...
data = data1 %>%
  ...
  dplyr::select(ID,Name,giraffeid,LATITUDE,LONGITUDE,dt,Operation,BATTERY,Park_location,Country,species,subspecies,sex,HACCURACY)#%>%
...
df = giraffe_pa %>%
  ...
  dplyr::select(ID,Name, LATITUDE,LONGITUDE,dt,Operation,Park_location,Country,species, subspecies, sex,NAME,DESIG)
...
HFP_trial = HFP_2009 %>%
  dplyr::filter(ID=="ST2010-2732")%>%
  dplyr::summarise(mean = mean(raster_HFP2009),
                   sd = sd(raster_HFP2009))
HFP_trial
#write.csv(HFP_2009, file="hfp_2009_2.csv")
...
#write.csv(human_density, file="human_density_2.csv")

human_density_trial = human_density %>%
  dplyr::filter(ID=="C31450")%>%
  dplyr::summarise(mean = mean(raster_human_density),
                   sd = sd(raster_human_density))
human_density_trial
